...
# Get ALL unique float_id_clean values from the table
print("\n📋 Fetching ALL unique float IDs from argo_measurements table...")
cursor.execute("SELECT DISTINCT float_id_clean FROM argo_measurements ORDER BY float_id_clean")
...
print("\n🗑️  Dropping existing argo_summary table if it exists...")
cursor.execute("DROP TABLE IF EXISTS argo_summary")

print("📦 Creating new argo_summary table...")
cursor.execute("""
CREATE TABLE argo_summary (
    float_id_clean TEXT PRIMARY KEY,
    summary TEXT
)
""")
print("✅ Table argo_summary created successfully!")
...
for fid in all_floats:
    ...
    try:
        ...
        # Insert into sample table (only float_id_clean and summary)
        cursor.execute(
            "INSERT INTO argo_summary (float_id_clean, summary) VALUES (%s, %s)", 
            (fid, summary)
        )
...
# Final verification
print("\n🔍 Verifying results...")
cursor.execute("SELECT COUNT(*) FROM argo_summary")
sample_count = cursor.fetchone()[0]
print(f"Entries in argo_summary table: {sample_count}")

# Show sample results
cursor.execute("SELECT float_id_clean, summary FROM argo_summary LIMIT 5")
sample_results = cursor.fetchall()

print("\n📋 Sample entries:")
print("float_id_clean | summary")
print("----------------+-----------------------------------------------------------------------------")
for fid, summary in sample_results:
    print(f"{fid:<15} | {summary}")

# Close connection
conn.close()
print("\n✅ Script completed successfully!")
print(f"🎉 argo_summary table created with {sample_count} floats!")
